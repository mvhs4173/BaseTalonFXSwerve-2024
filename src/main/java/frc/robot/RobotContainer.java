...
public class RobotContainer {
    ...
    private void configureButtonBindings() {
        ...
        ).withTimeout(7.0).finallyDo(() -> m_wrist2.holdPosition(0.215)); //wrist hold position must be the same as the wrist goto position
        Command goToAmpShotPosition = new ParallelCommandGroup(
            new ShoulderGoToPosition(m_shoulder, ShoulderGoToPosition.Method.kRPM, 10.0, -0.175),
            new Wrist2GoToPosition(m_wrist2, 0.35, 0.22)
        ).withTimeout(7.0).finallyDo(() -> m_wrist2.holdPosition(0.22)); //wrist hold position must be the same as the wrist goto position
